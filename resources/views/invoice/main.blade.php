    ...
    <form action="/customer/search" method="GET" class="form-inline bg-light ml-6">
      <div class="input-group input-group-sm m-1  ">
        <input class="form-control form-control-navbar  navbar-light badge-light @error('search') is-invalid @enderror" name='search' id="search" type="search" placeholder="Search Customer|min:4" aria-label="Search">
          {{-- @error('search')
          <a class="error invalid-feedback">{{ 'xxxxxxx' }}</a>
          @enderror --}}
...
{{--  </a> --}}
 <a class="nav-link" href="/warestart" >
        <i class="nav-icon fas fa-mobile-alt p-1 " > </i>
        <span class="badge badge-warning navbar-badge" data-toggle="tooltip" data-placement="top" title="WA Payment"> {{-- <b>{{App\Whatsapp::wa_payment()}} </b> --}}<i class="fa fa-retweet" aria-hidden="true"></i></span>
      </a>
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
       ...
       {{--  <li class="nav-item">
          <a href="{{ url ('customer')}}" class="nav-link">
           ...
           <p>
            ...
            <object  class="right badge badge-success"><a href="{{ url ('customer/create')}}">+ Add
            </a></object>
          </p>
        </a>
      </li> --}}
    ...
    <li class="nav-item has-treeview">
      ...
      <ul class="nav nav-treeview">
        <li class="nav-item">
          <a href="{{ url ('jurnal/rugilaba')}}" class="nav-link">
            <i class="far fa-circle nav-icon"></i>
            <p>Rugi Laba</p>
          </a>
        </li>
...
<script>
  ...
  $(document).ready(function() {
    $('form').submit(function() {
        $(this).find(':button[type=submit]').addClass("disabled").html('Processing..  <i class="fa fa-spinner fa-spin"></i>');
...
<script> 
  // $('#modal-ont').on('hidden.bs.modal', function () {
  //    window.location.reload();
  // });
  // $('#modal-ont').on('show.bs.modal', function () {
 ...
 $.ajax({
            ...
            method: 'post',
           data:{
            id_onu:document.getElementById("id_onu").value,
            id_olt:document.getElementById("id_olt").value
          },
           success:function(data){
              document.getElementById("ont_status").innerHTML=data;
           }
        });
// });
